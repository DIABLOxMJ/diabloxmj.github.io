let currentTab = localStorage.getItem('mcp_currentTab') || 'blocks'; 
		...
		function reloadConfig() {
			fetch('http://localhost:5000/reload-config', { method: 'POST' })
				.then(r => r.json())
				.then(data => {
					if (data.status === 'success') {
						alert("🎉 Configuration serveur rechargée avec succès !");
						runUpdateFinish(); // Rafraîchit les vues de l'IHM
					} else {
						alert("❌ Erreur lors du rechargement de la config : " + data.message);
					}
				})
				.catch(err => alert("❌ Impossible de joindre le serveur."));
		}
