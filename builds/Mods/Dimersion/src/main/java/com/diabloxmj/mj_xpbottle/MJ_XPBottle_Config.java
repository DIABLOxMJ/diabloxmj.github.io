...
public class MJ_XPBottle_Config {
    // Initialise le moteur GSON en lui ordonnant d'écrire le JSON de manière aérée et lisible (setPrettyPrinting)
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

    // Détermine l'emplacement physique du fichier de config sur le disque (ex: .minecraft/config/diabloxmj_xpbottle.json)
    private static final File CONFIG_FILE = new File(FabricLoader.getInstance().getConfigDir().toFile(), "diabloxmj_xpbottle.json");
    ...
    public static MJ_XPBottle_Config INSTANCE = new MJ_XPBottle_Config();
    ...
    public static void load() {
        ...
        if (CONFIG_FILE.exists()) {
            ...
            try (FileReader reader = new FileReader(CONFIG_FILE)) {
                ...
                INSTANCE = GSON.fromJson(reader, MJ_XPBottle_Config.class);
                // Sécurité : Si le fichier était corrompu ou vide, recrée une instance propre par défaut
                if (INSTANCE == null) {
                    INSTANCE = new MJ_XPBottle_Config();
                }
            } catch (IOException e) {
