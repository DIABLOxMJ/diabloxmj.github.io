...
public class XpBottleItem extends Item {
    ...
    // Variable textuelle interne servant à identifier le format de la fiole ("small", "medium", etc.)
    ...
    public XpBottleItem(Settings settings, String bottleType) {
    ...
    public int getMaxCapacity() {
        return switch (this.bottleType) {
            case "lvl1" -> ModConfig.INSTANCE.XPBottle_lvl1_Max_Capacity; // Capacité max configurée pour les petites fioles
            case "lvl2" -> ModConfig.INSTANCE.XPBottle_lvl2_Max_Capacity; // Capacité max configurée pour les fioles moyennes
            case "lvl3" -> ModConfig.INSTANCE.XPBottle_lvl3_Max_Capacity; // Capacité max configurée pour les grandes fioles
            case "lvl4" -> ModConfig.INSTANCE.XPBottle_lvl4_Max_Capacity; // Capacité max configurée pour les petites super fioles
            case "lvl5" -> ModConfig.INSTANCE.XPBottle_lvl5_Max_Capacity; // Capacité max configurée pour les super fioles moyennes
            case "lvl6" -> ModConfig.INSTANCE.XPBottle_lvl6_Max_Capacity; // Capacité max configurée pour les grandes super fioles
        ...
        if (!world.isClient() && player instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (serverPlayer.isSneaking()) {
                ...
                if (playerCurrentPool <= 0) {
                    serverPlayer.sendMessage(Text.translatable("chat.xpbank.player_empty"), true); // Message d'erreur discret dans l'actionbar
                ...
                if (currentStoredXp >= currentMaxCapacity) {
                    serverPlayer.sendMessage(Text.translatable("chat.xpbank.bottle_full"), true); // Message d'erreur indiquant que la fiole est pleine
                    return ActionResult.FAIL; // Retourne un échec de l'action
                }
                ...
                if (isLevelMode) {
                    ...
                    XpMathUtils.setPlayerTotalXp(serverPlayer, playerCurrentPool - toDeposit); // Applique le nouveau total de points réduits au joueur
                }
            ...
            // CAS 2 : Le joueur fait un Clic Droit simple -> Il RÉCUPÈRE l'XP de la fiole
                ...
                if (currentStoredXp <= 0) {
                    serverPlayer.sendMessage(Text.translatable("chat.xpbank.bottle_empty"), true); // Message indiquant que la fiole est vide
                    return ActionResult.FAIL; // Retourne un échec de l'action
                }
                ...
                if (isLevelMode) {
                    ...
                    XpMathUtils.setPlayerTotalXp(serverPlayer, playerCurrentPool + currentStoredXp); // Ajoute le contenu sous forme de points bruts
                }
    ...
    private boolean checkBottleIsLevelMode() {
        ...
        String type = switch (this.bottleType) {
            case "lvl1" -> ModConfig.INSTANCE.XPBottle_lvl1_Xp_Type;
            case "lvl2" -> ModConfig.INSTANCE.XPBottle_lvl2_Xp_Type;
            case "lvl3" -> ModConfig.INSTANCE.XPBottle_lvl3_Xp_Type;
            case "lvl4" -> ModConfig.INSTANCE.XPBottle_lvl4_Xp_Type;
            case "lvl5" -> ModConfig.INSTANCE.XPBottle_lvl5_Xp_Type;
            case "lvl6" -> ModConfig.INSTANCE.XPBottle_lvl6_Xp_Type;
